...
class EnvBase {
   public:
	...
	virtual bool step(const Ref<Vector<>> act, const Scalar dt, Ref<Vector<>> obs, Ref<Vector<>> reward) = 0;
	...
	virtual bool addRobotToUnity(const std::shared_ptr<UnityBridge> bridge);
	virtual bool addTrajectoryObjectToUnity(const std::shared_ptr<UnityBridge> bridge);
