...
Eigen::MatrixXi sign(const Matrix<Dynamic, Dynamic> x) { 
  return (x.array() > 0).cast<int>() - (x.array() < 0).cast<int>();
}

Scalar rad2deg(const Scalar angle) {
  return angle*180.0/M_PI;
}


Scalar deg2rad(const Scalar angle) {
