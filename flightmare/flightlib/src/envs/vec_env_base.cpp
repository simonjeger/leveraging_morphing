...
void VecEnvBase<EnvBaseName>::configEnv(const YAML::Node& cfg_node) {
	// initialization
	if (!cfg_node["unity"]["render"] || !cfg_node["simulation"]["seed"] || !cfg_node["unity"]["scene_id"] ||
	    !cfg_node["simulation"]["num_envs"] || !cfg_node["simulation"]["num_threads"]) {
		//
...
template <typename EnvBaseName>
bool VecEnvBase<EnvBaseName>::step(Ref<MatrixRowMajor<>> act, Ref<Vector<>> dt, Ref<MatrixRowMajor<>> obs, Ref<MatrixRowMajor<>> reward,
                                   Ref<BoolVector<>> done, Ref<MatrixRowMajor<>> extra_info) {
	if (act.rows() != num_envs_ || act.cols() != act_dim_ || dt.rows() != num_envs_ || dt.cols() != 1 || obs.rows() != num_envs_ || obs.cols() != obs_dim_ ||
	    reward.rows() != num_envs_ || reward.cols() != rew_dim_ || done.rows() != num_envs_ || done.cols() != 1 ||
	    extra_info.rows() != num_envs_ || extra_info.cols() != (long int)extra_info_names_.size()) {
		logger_.error("Input matrix dimensions do not match with that of the environment.");
...
template <typename EnvBaseName>
bool VecEnvBase<EnvBaseName>::getImage(Ref<ImgMatrixRowMajor<>> img, const bool rgb_image) {
	bool valid_img = true;
#pragma omp parallel for schedule(dynamic)
	for (int i = 0; i < num_envs_; i++) {
...
template <typename EnvBaseName>
bool VecEnvBase<EnvBaseName>::getDepthImage(Ref<DepthImgMatrixRowMajor<>> depth_img) {
	bool valid_img = true;
	for (int i = 0; i < num_envs_; i++) {
...
template <typename EnvBaseName>
void VecEnvBase<EnvBaseName>::perAgentStep(int agent_id, Ref<MatrixRowMajor<>> act, Ref<Vector<>> dt, Ref<MatrixRowMajor<>> obs,
                                           Ref<MatrixRowMajor<>> reward, Ref<BoolVector<>> done,
                                           Ref<MatrixRowMajor<>> extra_info) {
	// get individual rewards
	envs_[agent_id]->step(act.row(agent_id), dt(agent_id), obs.row(agent_id), reward.row(agent_id));
...
bool VecEnvBase<EnvBaseName>::setUnity(bool render) {
	...
	for (int i = 0; i < num_envs_; i++) {
		envs_[i]->addRobotToUnity(unity_bridge_ptr_);
		envs_[i]->addTrajectoryObjectToUnity(unity_bridge_ptr_);
	}
...
template <typename EnvBaseName>
void VecEnvBase<EnvBaseName>::isTerminalState(Ref<BoolVector<>> terminal_state) {}
...
template class VecEnvBase<RobotEnv>;
//
