...
void MiniPID::setDt(double dt_set){
	dt=dt_set;
	samplingFreq = 1/dt;
}
...
void MiniPID::storeReservoir(){
	errorSumSaved=errorSum;
}

void MiniPID::restoreReservoir(){
	reset();
	errorSum=errorSumSaved;
}
...
*/
double MiniPID::getOutput(double actual, double setpoint, bool useVel, double vel){

	// apply lowPass Filter
	// actual = lowPass(actual);
	...
	if(minOutput!=maxOutput && !bounded(output, minOutput,maxOutput) ){
		errorSum+=clamp(error,-maxIStep,maxIStep)*dt; 
		...
		errorSum+=clamp(error,-maxIStep,maxIStep)*dt;
		errorSum=clamp(errorSum,-maxError,maxError);
...
// double MiniPID::lowPass(double input){
// 	double RC = 1.0 / (2.0 * M_PI * cutoffFreq);
// 	double dt = 1.0 / samplingFreq;
// 	double alpha = dt / (RC + dt);
// 	double output = alpha * input + (1 - alpha) * prevOutputLP;
// 	prevOutputLP = output;
// 	return output;
// }